...
//================================================
{
  ...
  if ( (unsigned long)(currenttime - lasttime) >= 1000 ) {// toutes les secondes
    ...
    txtsd = txtsd + "," + printfloat2char(temp[2]) ;

    if ( (date_min != lastminute ) ) {// toutes les minutes
      lastminute = date_min;
  ...
  if (page == 1) {
    ...
    do {
      if (current_selection < 4) {
        ...
        u8g2.drawStr( 0, 12, printfloat2char(temp[2]));
        u8g2.drawStr(0, 30, unit_time);
        char txt[] = "Enreg#######";
        sprintf(txt, "Enreg# %d", nbEnreg);
        u8g2.drawStr(0, 40, txt);

        u8g2.drawStr(60, 40, FileSD);
